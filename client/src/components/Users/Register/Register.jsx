...
const validate = values => {
    ...
    if(!values.password){
        ...
        errors.password = 'Must be in between 5-20 char'
    }
...
const Register = () => {
  ...
  const {loading, appErr, serverErr, registered} = storeData.users;
            ...
            <div className="px-6 lg:px-20 py-12 lg:py-24 bg-gray-600 rounded-lg">
              <form onSubmit={formik.handleSubmit}>
                <h3 className="mb-10 text-2xl text-white font-bold font-heading">
                  Register Account
                  <div>
                    {appErr || serverErr ? <p className='text-red-400'>{appErr}, {serverErr}</p> : null}
                  </div>
                </h3>
                {/* First name */}
