...
import { updateUsersPasswordAction } from "../../../redux/slices/users/UsersSlices";
...
const UpdatePassword = () => {
  ...
  const {loading, isUpdated, userAuth} = user;
  //formik
  const formik = useFormik({
    ...
    onSubmit: values => {
      //dispath the action
      dispatch(updateUsersPasswordAction(values?.password))
    },
  });
  if(isUpdated){
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form className="space-y-6" onSubmit={formik.handleSubmit}>
            ...
            <div>
              ...
              {
                loading ? <button
                className="inline-flex bg-grey-700 justify-center w-full px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-200  hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
              >
